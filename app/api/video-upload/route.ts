...
//   api_key: process.env.CLOUDINARY_API_KEY,
//   api_secret: process.env.CLOUDINARY_API_SECRET,
//   timeout: 10 * 60 * 1000, // ✅ 10 min (prevents 499 timeout on slow network)
// });
...
//     if (!cloudName || !apiKey || !apiSecret) {
//       return NextResponse.json(
//         { error: "Cloudinary credentials not found (.env.local)" },
//         { status: 500 }
//       );
//     }
...
//     // ✅ IMPORTANT:
//     // Real "compressed size" needs a second derived asset generation.
//     // To keep uploads reliable, we save originalSize as compressedSize for now.
//     // Download/preview is still optimized via q_auto,f_mp4 URL.
//     const video = await prisma.video.create({
//       data: {
//         userId,
//         title,
...
//         publicId: uploaded.public_id,
//         originalSize: String(originalSize),
//         compressedSize: String(originalSize),
//         duration: Number(uploaded.duration ?? 0),
//       },
//     });
...
//     const msg =
//       error?.error?.message || // cloudinary style
//       error?.message ||
//       "Upload video failed";
