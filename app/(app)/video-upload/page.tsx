...
// function toNiceError(err: any): string {
//   // Cloudinary/Vercel/axios can return objects. React will crash if you render object.
...
//   // Cloudinary errors often look like: { error: { message, http_code, name } }
...
//   // UI limit (you can match Cloudinary preset limit)
//   const MAX_FILE_SIZE = 70 * 1024 * 1024;
...
//     try {
//       // ✅ 1) Upload DIRECTLY to Cloudinary (bypasses Vercel body limits)
//       const cloudForm = new FormData();
//       cloudForm.append("file", file);
//       cloudForm.append("upload_preset", uploadPreset);
//       // folder should be enforced by preset, but sending it is fine too:
//       cloudForm.append("folder", "video-uploads");
...
//       // ✅ 2) Save metadata in DB via our API (small JSON request)
//       await axios.post(
//         "/api/video-upload",
//         {
//           title: title.trim(),
...
//     } catch (err: any) {
//       console.error("Upload failed:", err);
//       setNotice({ type: "error", text: toNiceError(err) });
//     } finally {
//       setIsUploading(false);
//     }
//   };
...
export default function Page() {
  ...
  // UI limit (your Cloudinary plan may allow more)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ 1) Direct upload to Cloudinary (prevents Vercel 413)
      const cloudForm = new FormData();
      cloudForm.append("file", file);
      cloudForm.append("upload_preset", uploadPreset);
      cloudForm.append("folder", "video-uploads");
      ...
      const cloudRes = await axios.post(
        ...
        cloudForm,
        {
          timeout: 0, // don't timeout big uploads
          onUploadProgress: (evt) => {
            if (!evt.total) return;
            const pct = Math.round((evt.loaded / evt.total) * 100);
            setProgress(pct);
      ...
      const publicId: string = cloudRes.data.public_id;
      const bytes: number = Number(cloudRes.data.bytes ?? file.size);
      const duration: number = Number(cloudRes.data.duration ?? 0);

      // ✅ 2) Save metadata to DB (small JSON request)
      ...
      // ✅ accept both {id} and {video:{id}}
      const createdId = dbRes?.data?.id ?? dbRes?.data?.video?.id;
      if (!createdId) {
        console.log("Unexpected /api/video-upload response:", dbRes?.data);
        throw new Error(
          "Saved metadata but API did not return a video id. Check console."
        );
      }
